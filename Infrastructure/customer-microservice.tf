resource "aws_instance" "customer_msvc" {
	...
	provisioner "file" {
		source = "${var.CUSTOMERSVC_PATH}/target/${var.CUSTOMER_JAR}"
		destination = "~/${var.CUSTOMER_JAR}"
		connection {
			user = "ec2-user"
		}
	}
	...
	provisioner "file" {
		source = "${var.CUSTOMERSVC_PATH}/config.yml"
	...
	provisioner "remote-exec" {
		...
		"(crontab -l 2>/dev/null; echo '*/2 * * * * ~/RunMicroservice.sh ~/${var.CUSTOMER_JAR} >> microservice.log 2>&1') | crontab -"
		]
		connection {
			user = "ec2-user"
		}
